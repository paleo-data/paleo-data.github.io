...
| **Localities** | If any locality data should be [obscured](https://biokic.github.io/symbiota-docs/coll_manager/data_publishing/redaction/), include a _locationSecurity_ column in your spreadsheet and give records with sensitive locality data a value of “1”. | `0` |
| **Geologic Time** | Conform values to the [ICS Time Scale](https://stratigraphy.org/chart) and use “Early” in place of “Upper” and “Late” in place of “Lower”. Provide values for both _earlyInterval_ and _lateInterval_, even if both values are the same. | _earlyInterval_ = `Late Jurassic` and _lateInterval_ = `Early Cretaceous` |
