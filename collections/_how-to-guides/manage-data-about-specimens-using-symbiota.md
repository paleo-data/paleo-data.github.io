...
Regardless of data entry method, it is important that all data providers become familiar with the {% include glossary term="Darwin Core" namespace="" %}, which forms the basis for the majority of [Symbiota’s Data Fields](https://docs.symbiota.org/Editor_Guide/Editing_Searching_Records/symbiota_data_fields).
...
Mistakes are likely to happen, even in carefully curated datasets. It is therefore recommended that you routinely assess your data using the [Symbiota Data Quality Toolkit](https://docs.symbiota.org/Editor_Guide/data_quality_toolkit). This "toolkit" is designed to enable users with either Administrator or Editor permissions to your Collection Profile to “clean” your data–i.e. find and correct errors–using the portal’s built-in features wherever possible.
...
| **Basis of record** | Every record corresponding to cataloged fossil material should receive the {% include glossary term="basisOfRecord" namespace="dwc" %} value, "FossilSpecimen". <br>**Example:** _basisOfRecord_ = `FossilSpecimen` |
...
| **Dates** | Fields containing dates should be formatted in [ISO format](https://www.iso.org/iso-8601-date-and-time-format.html), e.g. YYYY-MM-DD. An exception to this rule is {% include glossary term="verbatimEventDate" namespace="dwc" %}; use this field when dates are incomplete or not ISO formatted. |
| **Identifications** | For specimens identified above the species level, do not include `sp.`, `indet.`, or similar suffixes. Qualifiers like `aff.` and `?` should be recorded in a separate field, {% include glossary term="identificationQualifier" namespace="dwc" %}. Verbatim label identifications (e.g. `Lepidophyllum?` can be captured in {% include glossary term="identificationRemarks" namespace="dwc" %}. Leave blank for specimens/specimen lots without identifications. See related information in [this tutorial](/tutorials/pdp-orientation#taxonomy--identificationsdeterminations). <br>**Example:** _scientificName_ = `Brachiopoda` + _identificationQualifier_ = `?` <br>**Example:** _scientificName_ = `Phylledestes vorax` + _scientificNameAuthorship_ = `Cockerell, 1907` |
...
| **Geological time** | Refer to Symbiota Docs for important [data field definitions](https://docs.symbiota.org/Editor_Guide/Editing_Searching_Records/symbiota_data_fields#paleontology-fields) and [related instructions](https://docs.symbiota.org/Editor_Guide/Editing_Searching_Records/paleontological_data#entering-chronostratigraphic-data). See important notice below regarding **verbatim values in geological context data**.<br>**Example:** _earlyInterval_ = `Pleistocene` + _lateInterval_ = `Pleistocene` <br>**Example:** _earlyInterval_ = `Asselian` + _lateInterval_ = `Sakmarian` + _localStage_ = `Wolfcampian` <br>**Example:** _earlyInterval_ = `Permian` + _lateInterval_ = `Permian` + _stratRemarks_ = `[VERBATIM CHRONOSTRATIGRAPHY: Permian?]` <br>**Example:** _earlyInterval_ = `Early Pennsylvanian` + _lateInterval_ = `Late Pennsylvanian` + _stratRemarks_ = `[VERBATIM CHRONOSTRATIGRAPHY: Pennsylvanian]` |
| **Geological units** | Refer to Symbiota Docs for important [data field definitions](https://docs.symbiota.org/Editor_Guide/Editing_Searching_Records/symbiota_data_fields#paleontology-fields) and see notice below regarding **verbatim values in geological context data**. <br>**Example:** _formation_ = `Wasatch` + _member_ = `Niland Tongue` <br>**Example:** _formation_ = `Chinle` + _stratRemarks_ = `[VERBATIM LITHOSTRATIGRAPHY: Dolores]` <br>**Example:** _group_ = `Cutler` + _formation_ = `Halgaito` + _stratRemarks_ = `[VERBATIM LITHOSTRATIGRAPHY: Cutler or Rico?]` |
| **Contextual descriptions** | Refer to the field definitions for [_description_](https://docs.symbiota.org/Editor_Guide/Editing_Searching_Records/symbiota_data_fields#description), [_element_](https://docs.symbiota.org/Editor_Guide/Editing_Searching_Records/symbiota_data_fields#element), and {% include glossary term="individualCount" namespace="dwc" %} for important information about how to format data related to anatomy and the physical nature of cataloged fossil material. <br>**Example:** see demo record for [USNMPAL144776](https://paleo.symbiota.org/portal/collections/individual/index.php?occid=763845) |
...
| **Cataloging multi-specimen lots** | When multiple individuals of a single taxon exist in a given lot (i.e. isolated in one physical container), they can be cataloged as a single occurrence record. [See example records](#example-records) for advice when a lot contains multiple taxa. |
| **Type specimens** | Include a value in {% include glossary term="typeStatus" namespace="dwc" %} ([ICZN](https://www.iczn.org/outreach/faqs/#faq-4) and [IAPT](https://www.iapt-taxon.org/nomen/pages/main/art_9.html) values preferred). [See below](#example-type-and-referred-specimens) for information about “extending” your specimens that are referenced in literature. |
| **Fields containing different kinds of information** | Record information that does not align with existing database fields using `key:value` pairs and, when necessary, concatenate this information into one field. {% include glossary term="occurrenceRemarks" namespace="dwc" %} and {% include glossary term="dynamicProperties" namespace="dwc" %} may be used for this purpose. <br>**Example:** _occurrenceRemarks_ = `ACQUISITION DETAILS: Gift of Arthur Lakes April 1890 | NOTES: Original specimen label misplaced` |
...
Some of the following records include redacted data (images and locality details); please contact [PaleoDataWG@gmail.com](mailto:PaleoDataWG@gmail.com) if you require more information about these records. Additionally, for ease of reference, these examples are organized by collection sub-discipline (e.g., "IP" = Invert Paleo, "VP" = Vert Paleo, etc.); however, many of these examples will apply across collection categories.
