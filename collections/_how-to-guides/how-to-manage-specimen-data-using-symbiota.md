...
| **Geological time** | Conform values to the [ICS Time Scale](https://stratigraphy.org/chart) and use “Early” in place of “Upper” and “Late” in place of “Lower”. Provide values for both _earlyInterval_ and _lateInterval_, even if both values are the same. See notice below for guidance related to verbatim values and uncertainty regarding to geological time.<br>**Example:** _earlyInterval_ = `Late Jurassic` and _lateInterval_ = `Early Cretaceous` |
| **Geological units** | Values for _group_, _formation_, _member_, and _bed_ should be parsed into [the appropriate fields](https://dwc.tdwg.org/terms/#dwc:formation). See notice below for guidance related to verbatim values and uncertainty regarding to geological units. <br>**Example:** _formation_ = `Wasatch Formation` and _member_ = `Niland Tongue` |
...
{: .notice--warning }
**A note on verbatim values in geological context data:** Many fossil specimens are accompanied by labels, field notes, and other primary data sources containing values that are no longer accepted (e.g. "Tertiary"), informally used (e.g. "Precambrian"), or indicate uncertaintly (e.g., "Upper Mio?"). This information provides important contextual data and should be recorded; however, it should not be captured Symbiota's _earlyInterval_ and _lateInterval_ fields, which map to a portal's standardized geological time scale values (which, by default, are based on the [ICS Time Scale](https://stratigraphy.org/chart)). In the absence of an appropriate, standard-based term to record these types of values, this information should be captured in _dynamicProperties_ as a key:value pair with a clear prefix. Regionally accepted values should be recorded in _localStage_ (e.g. "Bridgerian"). <br>**Example:** _VERBATIM CHRONOSTRATIGRAPHY_ = `Permian?`
