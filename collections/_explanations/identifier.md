...
Identifiers for physical objects, such as a fossil specimen or geologic sample, are commonly used as values in the biodiversity data standard terms dwc:materialEntityID, dwc:materialSampleID, and abcd:unitGUID. Think of these identifiers as a digital complement to human-friendly catalog numbers. 

UUIDs/GUIDs are very easy and inexpensive to acquire, and for this reason have been widely used by natural history collections to identify digital records for physical specimens in Darwin Core via dwc:occurrenceID. Although UUIDs/GUIDs will continue to be used and can persist as useful identifiers, the community is moving towards a preference for using true PIDs, such as [ARKs]((https://arks.org/).

Some identifiers are scoped specifically for referring to physical objects. For example, CETAF Stable Identifiers are typically assigned to specimens in natural history collections. CETAF (the [Consortium of European Taxonomic Facilities (CETAF)](https://cetaf.org/)) makes recommendations on the identifier format and qualities, but is not a true registration or resolution agency. Learn more about [CETAF's identifier practices here](https://cetafidentifiers.biowikifarm.net/wiki/Main_Page).

IGSNs (International Generic Sample Numbers) are functionally DOIs that are registered in a namespace dedicated to physical samples. Users can register samples and get IGSNs via [SESAR](geosamples.org), which is a community platform dedicated to managing and sharing metadata aboout physical samples. Learn about IGSNs [from DataCite](https://support.datacite.org/docs/igsn-ids), which is the parent registration agency that manages IGSNs. 
