...
# Portal Scope
The {% include resource_link filename='pdp.yml' %}’s scope is described on the home page, [https://paleo.symbiota.org](paleo.symbiota.org):
...
and is limited to extinct organisms and their traces (i.e., fossils) and collections that intend to use the portal for active data management.. The portal is intended for managing and sharing specimen data from fossil collections that can be used for research and are held in public trust, for example, by a university collection or nonprofit museum. The Paleo Data Portal is not intended for use by teaching collections, collections that are not publicly accessible, geological samples that do not contain fossils, archaeological or anthropological materials, or neontological (extant) specimen data.
