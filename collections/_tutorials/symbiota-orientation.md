...
# Portal scope
If you are interested in using the {% include resource_link filename='pdp.yml' %} to manage and share fossil data from your collection, first consider whether the portal's scope is suitable for your collection. The portal's scope is described on its home page and below:
...
Further, the portal is intended for _actively_ managing and sharing specimen data from fossil collections that can be used for research and are held in public trust, for example, by a university collection or nonprofit museum. It is not intended for use by teaching collections, private collections, and collections that are not publicly accessible. If the scope of this portal does not meet your needs, you might consider alternative collection management systems or creating your own Symbiota portal.

{% include resource_link filename='krimmel-2022-intro-digitization-3.yml' %}
...
# User support
## Technical
The Paleo Data Portal runs on {% include resource_link filename='symbiota.yml' %}, which is open source software used for creating themed data portals to manage and share biodiversity specimen data. Symbiota is based on the {% include resource_link filename='dwc-qrg.yml' %} data standard to enable easy data sharing with other Darwin Core-aligned databases. As noted above, the portal is hosted and maintained by the Symbiota Support Hub. The SSH develops the Symbiota code, maintains the software’s official user documentation, {% include resource_link filename='symbiota-documentation.yml' %}, and keeps the Paleo Data Portal’s software up-to-date (along with many [other](https://symbiota.org/symbiota-portals/) SSH-hosted Symbiota portals). The SSH also administers the portal’s underlying “backend” database and maintains its physical server at the University of Kansas. If you share data hosted on SSH-managed infrastructure, you should review the [SSH’s Data Sharing Policy](https://symbiota.org/data-sharing-policy/). 
...
## Data publishing 
Once a collection maintains catalog records in the Paleo Data Portal, sending a copy of this data to the [GBIF data portal](https://www.gbif.org/) is encouraged. Doing so will increase the visibility and impact of your data mobilization efforts ([examples](https://www.gbif.org/dataset/search?q=Paleontology&type=OCCURRENCE)). If applicable, data that are redacted in Symbiota will remain obscured in GBIF unless the data provider opts to change these settings in Symbiota at the time of data publication. Establishing the data publishing “pipeline” is typically straightforward, and once it’s set up, you can easily refresh your GBIF dataset directly from the Paleo Data Portal. The Symbiota Support Hub can assist with configuring data publishing for your collection in the Paleo Data Portal (recommended: CC [PaleoDataWG@gmail.com](PaleoDataWG@gmail.com)). 
